...
import {
	BusinessCustomerModuleData,
	ColumnBusinessCustomerModule,
...
function Customer({ id }: { id: number | undefined }) {
	...
	const dataBusinesses = () => {
		const tempArr: BusinessCustomerModuleData[] = [];
		businesses?.businessCustomers
			?.slice(0)
			.reverse()
			.forEach((business: any, index: number) => {
				return tempArr.push({
					...
					msisdn: business.msisdn,
					phonenumber: business.phonenumber,
					country: business.country,
					isblacklisted: (
						<StatusView
							status={business?.isblacklisted ? 'true' : 'false'}
							green='true'
							red='false'
						/>
					),
	...
	return (
		<div className={styles.containerHeader}>
			...
			<PaginationTable
				...
				columns={
					ColumnBusinessCustomerModule ? ColumnBusinessCustomerModule : []
				}
				emptyPlaceHolder={
