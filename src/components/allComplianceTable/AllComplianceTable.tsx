...
import styles from './AllComplianceTable.module.scss';
...
function AllComplianceTable() {
	...
	const downloadHandler = () => {
		ExportToXLS(rows, 'COMPLIANCE TABLEDATA');
	};
	...
	useEffect(() => {
		...
		aYAxios
			.get<BusinessTableApiTypes>(
				`/admin/business?perpage=${rowsPerPage}&page=${pageNumber}&fromdate=${fromDate}&todate=${toDate}&email=${email}&compliancestatus=${status}`
			)
			.then((res) => {
	...
	const columns: Column[] = [
		{ id: 'approved', label: 'Status', minWidth: 100 },
