...
const navbarReducer = (state = initialState, action:any) => {
  switch (action.type) {
    ...
    case "UPDATE_STARRED":
      const starredState = state.starred.map((i:any)=> i.id)
      const extraStarredState = state.extraStarred.map((j:any) => j.id)
      // Update main suggestions
      if (state) {
        state.suggestions.find((i:any) => {
      ...
      if (
        ...
        let newState = state.starred.push();
